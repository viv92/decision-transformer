...
class Embeddings(nn.Module):
    def __init__(self, d_model, K, o_dim, a_dim, num_actions, max_return, max_timestep, dropout):
        ...
        self.action_emb = nn.Embedding(num_actions, d_model) # +1 for pad_action
        self.returns2go_emb = nn.Embedding(int(max_return)+1, d_model) # +1 for pad_return2go = 0
...
if __name__ == '__main__':
    ...
    mean_eval_return_cap = 0 # udated from data
    collected_data_returns = []
    ...
    while len(dataset_states) < init_random_episodes:
        ...
        if ep_return > min_return:
            ...
            collected_data_returns.append(ep_returns2go[0])
            pbar.update(1)

    pbar.close()
    print('Data collected. Starting training...')

    # update mean_eval_return_cap from the collected data
    mean_eval_return_cap = sum(collected_data_returns) / len(collected_data_returns)
                ...
                for iter in range(eval_iters):
                    ...
                    while True:
                        ...
                        if action_idx < (K-1):
                            ...
                            eval_returns2go = eval_returns2go + [max(0, eval_returns2go[-1] - reward)]
                    ...
                    if ep_return > mean_eval_return_cap:
                        ...
                        dataset_returns2go.append(ep_returns2go)
                        dataset_timesteps.append(ep_timesteps)
                        print('len(dataset_states):{} \t mean_eval_return_cap:{}'.format(len(dataset_states), mean_eval_return_cap))
