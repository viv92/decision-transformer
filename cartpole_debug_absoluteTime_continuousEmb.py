...
class Embeddings(nn.Module):
    def __init__(self, d_model, K, o_dim, a_dim, num_actions, max_return, max_timestep, dropout):
        ...
        self.action_emb = nn.Linear(1, d_model, bias=False) # +1 for pad_action
        self.returns2go_emb = nn.Linear(1, d_model, bias=False) # +1 for pad_return2go = 0
        self.timestep_emb = nn.Linear(1, d_model, bias=False) # precautionary +1
...
def sample_minibatch(K, batch_size, dataset_states, dataset_actions, dataset_returns2go, dataset_timesteps, a_dim, env, pad_state, pad_return2go, device):
    ...
    minibatch_timesteps = torch.from_numpy(np.array(minibatch_timesteps)).float().to(device) # minibatch_timesteps.shape: [batch_size, K]
...
if __name__ == '__main__':
    ...
    lr = 1e-4
    num_epochs = 1000
    eval_freq = num_epochs//5
    # max_dataset_size = 10**5
    eval_iters = 5
    init_random_episodes = 10**4
    random_seed = 1010
    ...
    pbar = tqdm(total=init_random_episodes)
    while len(dataset_states) < init_random_episodes:
        ...
        #
        # if max_timestep < ep_timesteps[-1]:
        #     max_timestep = ep_timesteps[-1]

        # append to dataset
        if ep_return > min_return:
            # print('len(dataset_states): ', len(dataset_states))
            ...
            with torch.no_grad():
                ...
                for iter in range(eval_iters):
                    ...
                    while True:
                        ...
                        if render_final_episodes and (ep > num_epochs - 5):
                            env.render()
                        ...
                        input_timesteps = torch.from_numpy(np.array([eval_timesteps])).float().to(device)
                    ...
                    #     dataset_returns2go.append(ep_returns2go)
                    #     dataset_timesteps.append(ep_timesteps)
                    #     print('len(dataset_states):{} \t min_return:{}'.format(len(dataset_states), min_return))
                ...
                # eval_target_return = min(max_return, max_eval_return * 2)
