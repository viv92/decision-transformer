...
class DecisionTransformer(nn.Module):
    ...
    def predict(self, states, actions, returns2go, timesteps, action_idx, pad_mask):
        action_scores = self.forward(states, actions, returns2go, timesteps, pad_mask) # action_scores.shape: [batch_size=1, K, num_actions]
...
def init_decision_transformer(d_model, d_k, d_v, d_ff, n_heads, n_layers, o_dim, a_dim, num_actions, max_return, max_timestep, K, dropout, device):
    seq_len = 3 * K
...
if __name__ == '__main__':
    ...
    lr = 1e-4
    num_epochs = 20000
    eval_freq = 40 # so that agent gets time to improve on the data and is not just feeded new data during feedback
    eval_iters = 10 # so that mean_eval_return is a dependable value and not a high value by one-time fluke
    init_random_episodes = 4 # random dataset is constantly refreshed with new (better) datapoints
    init_expert_episodes = 0 # expert dataset is fixed and appended at each epsode
    max_dataset_size = 10 # so that dataset is ridden of very old values / behaviours
    random_seed = 1010
    ...
    expert_dataset_states, expert_dataset_actions, expert_dataset_returns2go, expert_dataset_timesteps = [], [], [], []
    ...
    dt_model = init_decision_transformer(d_model, d_k, d_v, d_ff, n_heads, n_layers, o_dim, a_dim, num_actions, max_return, max_timestep, K, dropout, device).to(device)
    ...
    for ep in tqdm(range(num_epochs)):
        ...
        # # resize dataset - select top k by ep_return
        # idx = np.arange(len(dataset_returns2go))
        # pairs = [ [idx[i],dataset_returns2go[i]] for i in range(len(idx)) ]
        # sorted_idx = [ k[0] for k in sorted(pairs, key=lambda x: x[1][0]) ]
        # topk_idx = sorted_idx[-max_dataset_size:]
        # dataset_states = [ dataset_states[i] for i in topk_idx ]
        # dataset_actions = [ dataset_actions[i] for i in topk_idx ]
        # dataset_returns2go = [ dataset_returns2go[i] for i in topk_idx ]
        # dataset_timesteps = [ dataset_timesteps[i] for i in topk_idx ]
        ...
        loss = calculate_cross_entropy_loss(minibatch_states, minibatch_actions, minibatch_returns2go, minibatch_timesteps, minibatch_pad_mask, dt_model, a_dim)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if (ep+1) % eval_freq == 0:
            ...
            with torch.no_grad():
                ...
                eval_returns = []
                delayed_dataset_states, delayed_dataset_actions, delayed_dataset_returns2go, delayed_dataset_timesteps = [], [], [], []
                for iter in range(eval_iters):
                    ...
                    while True:
                        ...
                        if action_idx < (K-1):
                            ...
                            eval_actions = eval_actions[:action_idx-1] + [action_scalar] + [env.action_space.sample()]
                            ...
                            eval_actions = eval_actions[1:-1] + [action_scalar] + [env.action_space.sample()]
                    ...
                    # append eval trajectories to dataset - if the return of the trajectory is greater than min_return
                    if ep_return > mean_eval_return_cap:
                        delayed_dataset_states.append(ep_states)
                        delayed_dataset_actions.append(ep_actions)
                        delayed_dataset_returns2go.append(ep_returns2go)
                        delayed_dataset_timesteps.append(ep_timesteps)
                        # print('mean_eval_return_cap:{:.3f} \t ep_return:{:.3f}'.format(mean_eval_return_cap, ep_return))
                ...
                if mean_eval_return_cap < mean_eval_return:
                    ...
                    dataset_states += delayed_dataset_states
                    dataset_actions += delayed_dataset_actions
                    dataset_returns2go += delayed_dataset_returns2go
                    dataset_timesteps += delayed_dataset_timesteps
