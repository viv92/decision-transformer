...
def sample_minibatch(K, batch_size, dataset_states, dataset_actions, dataset_returns2go, dataset_timesteps, a_dim, env, pad_state, pad_return2go, device):
    ...
    minibatch_states = torch.from_numpy(np.array(minibatch_states)).float().unsqueeze(-1).to(device) # minibatch_states.shape: [batch_size, K, o_dim]
...
if __name__ == '__main__':
    ...
    o_dim = 1 # state is an int

    max_timestep = env._max_episode_steps
    max_return = 1

    dataset_expert_return = 1
    dataset_expert_frac = .5
    ...
    while len(dataset_states) < init_random_episodes:
        ...
        # append to dataset
        if len(dataset_states) < ( init_random_episodes * (1 - dataset_expert_frac) ) or (ep_return >= dataset_expert_return):
            ...
            with torch.no_grad():
                ...
                for iter in range(eval_iters):
                    ...
                    while True:
                        ...
                        input_states = torch.from_numpy(np.array([eval_states])).float().unsqueeze(-1).to(device)
