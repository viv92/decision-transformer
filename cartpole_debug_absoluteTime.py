...
class Embeddings(nn.Module):
    def __init__(self, d_model, K, o_dim, a_dim, dropout):
        super().__init__()
        self.state_emb = nn.Linear(o_dim, d_model, bias=False)
        self.action_emb = nn.Linear(a_dim, d_model, bias=False)
        self.returns2go_emb = nn.Linear(1, d_model, bias=False)
        self.timestep_emb = nn.Linear(1, d_model, bias=False)
...
class DecisionTransformer(nn.Module):
    ...
    def predict(self, states, actions, returns2go, timesteps, action_idx):
        action_scores = self.forward(states, actions, returns2go, timesteps) # action_scores.shape: [batch_size=1, K, num_actions]
...
def init_decision_transformer(d_model, d_k, d_v, d_ff, n_heads, n_layers, o_dim, a_dim, num_actions, K, dropout, device):
    seq_len = 3 * K
    embeddings = Embeddings(d_model, K, o_dim, a_dim, dropout)
...
def calculate_cross_entropy_loss(minibatch_states, minibatch_actions, minibatch_returns2go, minibatch_timesteps, dt_model, a_dim):
    action_scores = dt_model(minibatch_states, minibatch_actions, minibatch_returns2go, minibatch_timesteps) # action_scores.shape: [batch_size, K, num_actions]
...
def sample_minibatch(K, batch_size, dataset_states, dataset_actions, dataset_returns2go, dataset_timesteps, a_dim, pad_action, device):
    ...
    minibatch_states = torch.from_numpy(np.array(minibatch_states, dtype=np.float32)).to(device) # minibatch_states.shape: [batch_size, K, o_dim]
    minibatch_actions = torch.from_numpy(np.array(minibatch_actions, dtype=np.float32)).to(device) # minibatch_actions.shape: [batch_size, K]
    minibatch_returns2go = torch.from_numpy(np.array(minibatch_returns2go, dtype=np.float32)).to(device) # minibatch_returns2go.shape: [batch_size, K]
    minibatch_returns2go = minibatch_returns2go.unsqueeze(-1) # minibatch_returns2go.shape: [batch_size, K, 1]
    minibatch_timesteps = torch.from_numpy(np.array(minibatch_timesteps, dtype=np.float32)).to(device) # minibatch_timesteps.shape: [batch_size, K]
    ...
    return minibatch_states, minibatch_actions, minibatch_returns2go, minibatch_timesteps
...
if __name__ == '__main__':
    ...
    dt_model = init_decision_transformer(d_model, d_k, d_v, d_ff, n_heads, n_layers, o_dim, a_dim, num_actions, K, dropout, device).to(device)
    ...
    for ep in tqdm(range(num_epochs)):
        ...
        minibatch_states, minibatch_actions, minibatch_returns2go, minibatch_timesteps = sample_minibatch(K, batch_size, dataset_states, dataset_actions, dataset_returns2go, dataset_timesteps, a_dim, pad_action, device)

        # calculate loss (CrossEntropyLoss as action is discrete in this env) and take gradient step
        loss = calculate_cross_entropy_loss(minibatch_states, minibatch_actions, minibatch_returns2go, minibatch_timesteps, dt_model, a_dim)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if (ep+1) % (num_epochs // eval_freq) == 0:
            ...
            with torch.no_grad():
                ...
                timestep = 0
                ep_states, ep_actions, ep_rewards, ep_timesteps = [], [], [0.], [] # first reward should be 0
                eval_states, eval_actions, eval_returns2go, eval_timesteps = [state], [pad_action], [max_return], [0] # target_return = max_return; pad_action is a placeholder for predicted action
                ...
                while True:
                    ...
                    input_states = torch.from_numpy(np.array([eval_states], dtype=np.float32)).to(device)
                    input_actions = torch.from_numpy(np.array([eval_actions], dtype=np.float32)).to(device)
                    input_returns2go = torch.from_numpy(np.array([eval_returns2go], dtype=np.float32)).to(device)
                    input_returns2go = input_returns2go.unsqueeze(-1)
                    input_timesteps = torch.from_numpy(np.array([eval_timesteps], dtype=np.float32)).to(device)
                    input_timesteps = input_timesteps.unsqueeze(-1)
                    if a_dim == 1:
                        input_actions = input_actions.unsqueeze(-1)

                    # get predicted action
                    action = dt_model.predict(input_states, input_actions, input_returns2go, input_timesteps, action_idx)
